...
@Component
public class SearchEmployeeByName implements IEmployeeSearchStrategy {
    @Override
    public List<Employee> searchData(String searchStr) {
        return Employee.getEmployees().stream().filter(employee ->
                employee.getFirstName().equalsIgnoreCase(searchStr)
        ).collect(Collectors.toList());
    }

    @Override
    public SearchType getSearchType() {
        return SearchType.EMPLOYEE_SEARCH_BY_NAME;
    }
}
